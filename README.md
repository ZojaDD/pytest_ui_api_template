...
Проект содержит 5 UI и 6 API автотестов для веб-приложения Кинопоиск.

## Особенности реализации
**Кросс-браузерная совместимость**: Настройки Chrome оптимизированы для обхода детекции автоматизации (из-за специфики защиты сайта необходимо проходить проверки на капчу вручную)
...
pytest_ui_api_template/
├── tests/
│   ├── test_kinopoisk_ui.py          # UI-тесты для веб-интерфейса Кинопоиска
│   └── test_kinopoisk_api.py         # API-тесты для Kinopoisk API
...
### UI-тесты (test_kinopoisk_ui.py):

- **test_ui_main_page_load** - Проверка загрузки главной страницы Кинопоиска и основных элементов
- **test_ui_search_domovenok_kuzya** - Тестирование поиска фильма "Домовенок Кузя" через интерфейс
...
### API-тесты (test_kinopoisk_api.py):

- **test_api_key_valid** - Проверка валидности API ключа
- **test_api_search_house_domovenok_kuzya** - Поиск фильма "Домовенок Кузя" через API
